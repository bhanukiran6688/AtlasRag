...
class DocumentSplitter:
    ...
    def split_documents(
        self, documents: list[Document], file_type: str | None = None
    ) -> list[Document]:
        """
        ...
        if file_type is not None and file_type.lower() in ("csv", "json"):
            logger.info(
                "Skipping recursive splitting for structured data format: %s", file_type
            )
            return documents
    ...
    def _split_documents_parent_child(
        ...
        logger.info(
            "Processed %d chunks (%d parent, %d child) with parent-child retrieval.",
            len(all_chunks),
            len(parent_chunks),
