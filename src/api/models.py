...
class RAGRequest(BaseModel):
    ...
    use_query_expansion: bool = Field(
        default=False, description="Enable query expansion"
    )
    use_query_decomposition: bool = Field(
        default=False, description="Enable query decomposition"
    )
    metadata_filter: Optional[Dict[str, Any]] = Field(
        default=None, description="Optional metadata filter for retrieval"
    )
...
class Claim(BaseModel):
    ...
    citations: List[Citation] = Field(
        default_factory=list, description="Citations supporting the claim"
    )
...
class RetrievedChunk(BaseModel):
    ...
    rerank_score: Optional[float] = Field(
        default=None, description="Rerank score if applicable"
    )
...
class RAGResponse(BaseModel):
    ...
    llm_latency_ms: Optional[float] = Field(
        default=None, description="LLM latency in milliseconds"
    )
    total_tokens: int = Field(default=0, description="Total tokens used")
    cost_usd: Optional[float] = Field(default=None, description="Cost in USD")
    sanitized_question: Optional[str] = Field(
        default=None, description="Sanitized question after PII redaction"
    )
    is_blocked: bool = Field(
        default=False, description="Whether the request was blocked"
    )
    blocked_reason: Optional[str] = Field(
        default=None, description="Reason for blocking"
    )
    pii_detected: List[str] = Field(
        default_factory=list, description="PII fields detected"
    )
    ...
    structured_output: Optional[Dict[str, Any]] = Field(
        default=None, description="Structured LLM output"
    )
    error: Optional[str] = Field(
        default=None, description="Error message if applicable"
    )
    retrieval_queries: Optional[List[str]] = Field(
        default=None, description="Queries used for retrieval"
    )
    is_grounded: bool = Field(
        default=True, description="Whether the answer is grounded in context"
    )
    grounding_reason: Optional[str] = Field(
        default=None, description="Reasoning for grounding decision"
    )
    grounding_confidence: Optional[float] = Field(
        default=None, description="Grounding confidence score"
    )
