...
async def test_rag_service_aprocess_basic(mock_dependencies):
    ...
    # Setup mocks
    doc = Document(page_content="RAG is cool", metadata={"source": "test.txt"})
    ...
    llm_gateway.generate.return_value = LLMResponse(
        content='{"answer": "RAG is cool", "citations": [], "claims": []}',
        model="test-model",
        latency_ms=50.0,
