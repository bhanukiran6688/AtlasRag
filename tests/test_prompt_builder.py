...
class TestContextBuilder:
    ...
    def test_build_removes_duplicate_chunks(self, monkeypatch):
        monkeypatch.setattr(
            ...
            Mock(max_context_tokens=1000, context_compression_max_sentences=3),
        )
    ...
    def test_trim_context_to_chunks_returns_full_context_when_under_limit(
        self, monkeypatch
    ):
        monkeypatch.setattr(
            "src.config.settings.settings",
...
class TestPromptBuilder:
    ...
    def test_build_includes_conversation_history(self, monkeypatch):
        ...
        result = PromptBuilder.build(
            "test question", "test context", conversation_history=history
        )
    ...
    def test_build_handles_empty_conversation_history(self, monkeypatch):
        ...
        result = PromptBuilder.build(
            "test question", "test context", conversation_history=None
        )
