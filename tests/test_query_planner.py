...
class TestQueryPlanner:
    ...
    def test_build_queries_returns_original_when_disabled(self, query_planner, monkeypatch):
    ...
    def test_should_expand_returns_true_when_adaptive_disabled(self, query_planner, monkeypatch):
        monkeypatch.setattr("src.config.settings.settings", Mock(
            query_planning_adaptive_enabled=False
        ))
    ...
    def test_should_expand_with_high_confidence_and_short_question(self, query_planner, monkeypatch):
        monkeypatch.setattr("src.config.settings.settings", Mock(
            query_planning_adaptive_enabled=True,
            query_planning_low_confidence_threshold=0.5,
            query_planning_min_words_for_expansion=10
    ...
    def test_should_decompose_with_multi_part_question(self, query_planner, monkeypatch):
        monkeypatch.setattr("src.config.settings.settings", Mock(
            query_planning_adaptive_enabled=True,
            query_planning_low_confidence_threshold=0.5,
    ...
    def test_heuristic_variants_generates_alternatives(self, query_planner, monkeypatch):
        monkeypatch.setattr("src.config.settings.settings", Mock(
    ...
    def test_heuristic_subquestions_splits_on_conjunctions(self, query_planner, monkeypatch):
        monkeypatch.setattr("src.config.settings.settings", Mock(
    ...
    def test_heuristic_subquestions_with_simple_question(self, query_planner, monkeypatch):
        monkeypatch.setattr("src.config.settings.settings", Mock(
            query_decomposition_max_subquestions=3
        ))
    ...
    def test_extract_string_list_with_valid_json(self):
        result = QueryPlanner._extract_string_list('{"queries": ["q1", "q2"]}', "queries", 5)
        assert result == ["q1", "q2"]
    ...
    def test_extract_string_list_respects_limit(self):
        result = QueryPlanner._extract_string_list('{"queries": ["q1", "q2", "q3", "q4"]}', "queries", 2)
        assert len(result) == 2
    ...
    @pytest.mark.asyncio
    async def test_generate_variants_falls_back_on_error(self, query_planner, mock_gateway, monkeypatch):
        monkeypatch.setattr("src.config.settings.settings", Mock(
            query_expansion_max_variants=2
        ))
    ...
    @pytest.mark.asyncio
    async def test_generate_subquestions_falls_back_on_error(self, query_planner, mock_gateway, monkeypatch):
        monkeypatch.setattr("src.config.settings.settings", Mock(
            query_decomposition_max_subquestions=2
        ))
