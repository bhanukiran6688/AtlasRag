...
class TestDocumentSplitter:
    def test_splitter_raises_error_when_overlap_equals_size(self):
        with patch("src.config.settings.settings.chunk_size", 100), \
             patch("src.config.settings.settings.chunk_overlap", 100):
            with pytest.raises(
                ValueError, match="chunk_overlap must be smaller than chunk_size"
            ):
                DocumentSplitter()

    def test_splitter_raises_error_when_overlap_exceeds_size(self):
        with patch("src.config.settings.settings.chunk_size", 100), \
             patch("src.config.settings.settings.chunk_overlap", 150):
            with pytest.raises(
                ValueError, match="chunk_overlap must be smaller than chunk_size"
            ):
                DocumentSplitter()

    def test_split_documents_returns_empty_list_for_empty_input(self):
        with patch("src.config.settings.settings.chunk_size", 100), \
             patch("src.config.settings.settings.chunk_overlap", 20):
            splitter = DocumentSplitter()
            result = splitter.split_documents([])
            assert result == []

    def test_split_documents_skips_csv_format(self):
        with patch("src.config.settings.settings.chunk_size", 100), \
             patch("src.config.settings.settings.chunk_overlap", 20):
            splitter = DocumentSplitter()
    ...
    def test_split_documents_skips_json_format(self):
        with patch("src.config.settings.settings.chunk_size", 100), \
             patch("src.config.settings.settings.chunk_overlap", 20):
            splitter = DocumentSplitter()
    ...
    def test_split_documents_splits_long_text(self):
        with patch("src.config.settings.settings.chunk_size", 50), \
             patch("src.config.settings.settings.chunk_overlap", 10):
            splitter = DocumentSplitter()
            long_text = (
